...
window.SRSEngine = {
  ...
  /**
   * 全データから「今日復習すべき問題リスト」を抽出
   */
  getDueItems(words = [], verbs = []) {
    const now = Date.now();
    const dueWords = words.filter(w => {
      const state = window.StorageEngine.getItem(w.id);
    ...
    return { dueWords, dueVerbs, totalDue: dueWords.length + dueVerbs.length };
  },

  /**
   * 全データから「弱点アイテム（過去誤答数 > 0 の未克服問題）」を抽出
   */
  getWeakItems(words = [], verbs = []) {
    const allItems = [
      ...words.map(w => ({ ...w, itemType: 'word' })),
      ...verbs.map(v => ({ ...v, itemType: 'verb' }))
    ];
   ...
   * 全体の記憶定着度ステータスを計算
   */
  getStats(words = [], verbs = []) {
    const allItems = [...words, ...verbs];
